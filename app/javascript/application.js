...
const sanitizeMazeFileNamePart = (value = "") => {
  return String(value)
    .trim()
    .replace(/[\\/:*?"<>|]/g, "-")
    .replace(/\s+/g, "_")
...
const buildMazeDownloadFileName = ({
  kind = "question",
  title = "",
  generatedAt = ""
} = {}) => {
...
const convertSvgMarkupToPngBlob = async (
  svgMarkup = "",
...
const downloadBlobFile = (blob, fileName = "maze.png") => {
  if (!blob) return
...
const showDownloadButtonFeedback = (button, label = "保存開始") => {
  if (!button) return
